...
$sqlInsert = "DELETE FROM Taller_socio WHERE taller ='". $_GET["taller"] . "' AND socio = '" . $_GET["DNI"]. "'";
...
//si no hay resultados, avisar que no hay socio con ese DNI
	if ($result->num_rows < 1) 	
		...
		header('Location: /interfaz/quitar_socio_de_taller.php?respuesta=No_existe'); 
		

	else if ($result2->num_rows < 1) 	
		
		header('Location: /interfaz/quitar_socio_de_taller.php?respuesta=No_existe2'); 
	...
	if($result3->num_rows > 0){
		...
		header('Location: /interfaz/quitar_socio_de_taller.php?respuesta=OK'); 
	
	}

	else header('Location: /interfaz/quitar_socio_de_taller.php?respuesta=NO_Existe');
